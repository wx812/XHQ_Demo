...
public class ProcessUtils {
    ...
    public static ActivityManager getActMgr(){
        return (ActivityManager)AppUtils.getAppContext().getSystemService(Context.ACTIVITY_SERVICE);
    }
    ...
    public static String getForegroundProcessName() {
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager pkgMgr = AppUtils.getPkgMgr();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = pkgMgr.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
//                try {
                    ApplicationInfo info = AppUtils.getAppInfo();
...
//                        LogUtils.d("getForegroundApp", "没有打开\"有权查看使用权限的应用\"选项");
                    ...
                    UsageStatsManager usMgr = (UsageStatsManager) ctx.getSystemService(Context.USAGE_STATS_SERVICE);
                    if(usMgr == null) return null;
                    long endTime = System.currentTimeMillis();
                    long beginTime = endTime - 86400000 * 7;
                    List<UsageStats> uStats = usMgr.queryUsageStats(UsageStatsManager.INTERVAL_BEST, beginTime, endTime);
                    if (uStats == null || uStats.isEmpty()) return null;
                    UsageStats recentStats = null;
                    for (UsageStats us : uStats) {
                        if (recentStats == null || us.getLastTimeUsed() > recentStats.getLastTimeUsed()) {
                            recentStats = us;
                        }
                    }
