...
public class EncodeUtils{
    ...
    /**
     * give the source string transcoding
     *
     * @param sourceStr source string
     * @param charsetName char set name
     * @return after the transcoding string
     */
    public static String transCodeCharset(String sourceStr, String charsetName){
        String strFormat = "";
        try{
            boolean iso_str = Charset.forName("ISO-8859-1").newEncoder().canEncode(sourceStr);
            boolean gbk_str = Charset.forName("GBK").newEncoder().canEncode(sourceStr);
            boolean utf8_str = Charset.forName("UTF-8").newEncoder().canEncode(sourceStr);
            if(iso_str){
                strFormat = new String(sourceStr.getBytes("ISO-8859-1"), charsetName);
            }else if(gbk_str){
                strFormat = new String(sourceStr.getBytes("GBK"), charsetName);
            }else if(utf8_str){
                strFormat = new String(sourceStr.getBytes("UTF-8"), charsetName);
            }else{
                strFormat = sourceStr;
            }
