...
import com.xhq.demo.db.db_greendao.gen.ChatMessageBeanDao;
import com.xhq.demo.db.db_greendao.gen.DaoMaster;
import com.xhq.demo.db.db_greendao.gen.DaoSession;
...
public abstract class BaseManager<M, K> implements IDatabase<M, K> {
    private static final String DEFAULT_DATABASE_NAME = "home.db";
    ...
    @Override
    public long getPages(int number, @NonNull String id, @NonNull String user_id) {
//        long count = getAbstractDao().queryBuilder().count();
        long count = new ChatMsgDbManager().getAbstractDao().queryBuilder().where(
                ChatMessageBeanDao.Properties.Uid.eq(id),
                ChatMessageBeanDao.Properties.UserId.eq(user_id)).count();
