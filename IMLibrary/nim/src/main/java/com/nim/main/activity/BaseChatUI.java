...
public abstract class BaseChatUI extends AppCompatActivity {
     ...
     * 初始化状态栏
     */
    protected void initStateBarColor() {
        // 4.4及以上版本开启
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (getStateBarColor() == 0) {
                tintManager.setTintColor(Color.WHITE);
            } else {
                tintManager.setTintColor(getStateBarColor());
            }
        }
    }
    ...
    public void setTintBarColor(int color) {
        if (null != tintManager) {
            tintManager.setTintColor(color);
        }
    }

    public void setTintBarRes(int resId) {
        if (null != tintManager) {
