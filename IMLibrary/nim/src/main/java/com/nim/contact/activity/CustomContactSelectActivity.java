 ...
 */
public class CustomContactSelectActivity extends UI implements View.OnClickListener, SearchView.OnQueryTextListener, AddFriendDialogFragment.ItemClickCallBack {
    ...
    private List<ContactItem> selects = new ArrayList<>();
    ...
    public void onItemClickSure(List<String> selected, String msg) {
        if (TextUtils.isEmpty(msg)) {
            TeamCreateHelper.createAdvancedTeam(CustomContactSelectActivity.this, selected);
        } else {
            TeamCreateHelper.createAdvancedTeam(CustomContactSelectActivity.this, msg, selected);
        }
    ...
    public static class CustomOption implements Serializable {
         ...
         * 是否支持全选(默认不支持)
         */
        public boolean selectAllVisible = false;
        ...
        /**
         * 需要disable(可见但不可操作）的联系人id
         */
        public String disableFilterId = null;

    }

    public static void startActivityForResult(Context context, CustomOption option, int requestCode) {
        Intent intent = new Intent();
        intent.putExtra(EXTRA_DATA, option);
        intent.setClass(context, CustomContactSelectActivity.class);

        ((Activity) context).startActivityForResult(intent, requestCode);
    }
    public static void startActivity(Context context, CustomOption option) {
        Intent intent = new Intent();
        intent.putExtra(EXTRA_DATA, option);
        intent.setClass(context, CustomContactSelectActivity.class);
    ...
    private void initActionbar() {
        ...
        toolbarView.setVisibility(!option.selectAllVisible ? View.GONE : View.VISIBLE);
        toolbarView.setText(R.string.select);
        toolbarView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (contacts != null && !contacts.isEmpty()) {
                    if (toolbarView.getText().toString().equals("全选")) {
                        toolbarView.setText(R.string.unselect);
                        contactAdapter.setAlreadySelectedAccounts(contacts);
                        List<ContactItem> selectedItems = contactAdapter.getSelectedItem();
                        for (ContactItem item : selectedItems) {
                            contactSelectedAdapter.removeContact(item.getContact());
                        ...
                        List<ContactItem> selectedItems = contactAdapter.getSelectedItem();
                        for (ContactItem item : selectedItems) {
                            contactAdapter.cancelItem(item.getContact());
                            contactSelectedAdapter.removeContact(item.getContact());
                        }
                        arrangeSelected();
                    }
    ...
    private void parseIntentData() {
        this.option = (CustomOption) getIntent().getSerializableExtra(EXTRA_DATA);
            ...
            if (!loadedTeamMember) {
                ...
                for (AbsContactItem item : data) {
                    if (item instanceof ContactItem) {
                        ...
                        if (!contact.getDisplayName().equals("我")) {
    ...
    private void initAdapter() {
        ...
        // contact adapter
        contactAdapter = new ContactSelectAdapter(CustomContactSelectActivity.this, new ContactsSelectGroupStrategy(),
                dataProvider) {
        ...
        contactSelectedAdapter = new ContactSelectAvatarAdapter(this, option.type == ContactSelectType.TEAM);
    }
                ...
                if (option.multi) {
                    ...
                    if (contactAdapter.isSelected(position)) {
                        ...
                        if (contactSelectedAdapter.getCount() <= option.maxSelectNum) {
                            ...
                            Toast.makeText(CustomContactSelectActivity.this, option.maxSelectedTip, Toast.LENGTH_SHORT).show();
                        }
        ...
        imageSelectedGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    ...
                    IContact item = (IContact) contactSelectedAdapter.getItem(position);
                    if (!TextUtils.isEmpty(option.disableFilterId)) {
                        if (option.disableFilterId.equals(item.getContactId())) {
                            return;
                        }
                    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btnSelect) {
            ...
            if (option.allowSelectEmpty || checkMinMaxSelection(contacts.size())) {
                ...
                if (!option.isCreateTeam){
                    onSelected(selectedAccounts);
                }else{
                    //创建群聊 选择联系人
                    AddFriendDialogFragment dialogFragment = AddFriendDialogFragment.newInstance(selectedAccounts, "创建群聊", "请输入群聊名称(限制10个字以内)");
                    dialogFragment.setListener(this);
                    dialogFragment.setStyle(DialogFragment.STYLE_NO_TITLE, android.R.style.Theme_Holo_Light_Dialog_MinWidth);
                    dialogFragment.show(CustomContactSelectActivity.this.getSupportFragmentManager(), "dialog");
                }
