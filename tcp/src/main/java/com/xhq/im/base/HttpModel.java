...
import com.xhq.common.tool.ErrorMessage;
import com.xhq.common.tool.SnbUtils;
import com.xhq.common.tool.UtilEncode;
import com.xhq.common.tool.appTool.AppDirUtil;
import com.xhq.im.base.bean.AllBean;
import com.xhq.im.tcp.custom.TcpServiceSender;
...
//                    Logger.t(requestType + "-解密之后的data数据").d(bitmap);
    ...
    protected void downloadFile(String url, Map<String, String> params, final String requestType) {
        ...
        String jmzw_download = AppDirUtil.getInternalCacheDir("JMZW_DOWNLOAD").getAbsolutePath();
    ...
    private void commonHandle(String requestType, Observable<Response<String>> observable, Map<String, String>
        ...
        observable.observeOn(Schedulers.computation())
                .map((Response<String> stringResponse) -> {
                    ...
                    if (mAllBean.isOk()) {
                        if (mAllBean.getSc().equals(SnbUtils.getSecurityCode(mAllBean.getData()))) {
                            return UtilEncode.base64DecodeEx(mAllBean.getData());
                        } else {
                            throw new RuntimeException("未授权用户");
                        }
                    } else {
                        throw new RuntimeException(mAllBean.getMsg());
                    }
                })
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
